...
def go(args):
    ...
    if not isinstance(X, pd.DataFrame):
        logger.error("Could not read the training data")
        raise ValueError("trainval_artifact must be a CSV file")
    ...
    model_rf_dir = "random_forest_dir"
    if os.path.exists(model_rf_dir):
        shutil.rmtree(model_rf_dir)

    ######################################
    # Save the sk_pipe pipeline
    ######################################
    mlflow.sklearn.save_model(sk_pipe, model_rf_dir)

    # Upload the model we just exported to W&B
    artifact = wandb.Artifact(name=args.output_artifact, type="model_export",
                              description="Random Forest Model", metadata=rf_config)
    artifact.add_dir(model_rf_dir)
